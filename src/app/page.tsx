    ...
    <div className="space-y-24 pb-24 pt-10">
      ...
      <section id="solutions"><Container><SectionTitle eyebrow="Решения" title="Что мы делаем" description="Приоритетные продуктовые направления для роста и эффективности." />
        <div className="grid gap-4 md:grid-cols-2">
          {solutions.slice(0, 4).map((s: Solution, i: number) => <Reveal key={s.slug} delay={i * 0.05}><article className="glass rounded-2xl p-6"><p className="mb-2 text-xs text-primary">#{s.priority}</p><h3 className="mb-2 text-2xl">{s.title}</h3><p className="text-muted">{s.summary}</p></article></Reveal>)}
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">{caseItems.slice(0,6).map((c: CaseItem)=><article key={c.company} className="glass rounded-2xl p-5"><h3 className="text-xl">{c.company}</h3><p className="text-sm text-primary">{c.industry}</p><p className="mt-2 text-sm text-muted">{c.challenge}</p><p className="mt-2 text-sm">{c.impact}</p></article>)}</div>
      ...
      <div className="space-y-3">{faq.map((f: FaqItem)=><details key={f.question} className="glass rounded-xl p-4"><summary className="cursor-pointer">{f.question}</summary><p className="mt-2 text-sm text-muted">{f.answer}</p></details>)}</div></Container></section>

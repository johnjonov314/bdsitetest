  ...
  return (
    <div className="space-y-24 pb-24 pt-10">
        ...
        <Container>
          <Reveal>
            <div className="noise relative space-y-8 rounded-3xl border border-white/10 px-6 py-16 md:px-12">
              <p className="font-mono text-xs uppercase tracking-[0.24em] text-primary">Enterprise AI Platform • Russia</p>
              <h1 className="max-w-5xl text-5xl font-semibold leading-tight md:text-7xl">Beeline Big Data & AI</h1>
              <p className="max-w-3xl text-lg text-muted">
                Данные оператора, AI-модели и инженерная экспертиза для enterprise и госсектора: надежно, юридически корректно,
                с фокусом на внедрение и измеримый результат.
              </p>
      ...
      <section id="solutions">
        <Container>
          <SectionTitle eyebrow="Решения" title="Что мы делаем" description="Приоритетные направления и контентные продуктовые страницы для каждой линии решений." />
            ...
            {solutions.slice(0, 4).map((s: Solution, i: number) => (
              <Reveal key={s.slug} delay={i * 0.05}>
                <article className="glass noise rounded-2xl p-6">
                  ...
                  <Button href={`/solutions/${s.slug}`} variant="secondary" className="mt-5">
      ...
      <section>
        <Container>
          <SectionTitle
            eyebrow="Spotlight"
            title="ИИ-агенты"
            description="Автоматизируют повторяемые операции, сокращают время решения задач и повышают предсказуемость процессов."
          />
      ...
      <section>
        <Container>
          <SectionTitle
            ...
            description="Безопасность объектов, контроль инцидентов и качество процессов в реальном времени."
          />
          <div className="glass grid gap-4 rounded-2xl p-6 md:grid-cols-3">
            {["Контроль периметра", "Детекция инцидентов", "Стандарты процессов"].map((x) => (
              <p key={x} className="rounded-xl border border-white/10 p-4">{x}</p>
            ))}
          </div>
        </Container>
      </section>
      ...
      <section id="industries">
        <Container>
          <SectionTitle eyebrow="Отрасли" title="Фокус по индустриям" />
          <div className="grid-pattern rounded-2xl border border-white/10 p-6">
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-4">
              {industries.map((i) => <span key={i} className="glass rounded-lg px-3 py-2 text-sm">{i}</span>)}
            </div>
          </div>
        </Container>
      </section>
      ...
      <section>
        <Container>
          <SectionTitle eyebrow="Delivery" title="Как внедряем" />
          <div className="grid gap-4 md:grid-cols-4">
            {["Discovery", "Пилот", "Интеграция", "Поддержка/SLA"].map((x, idx) => (
              <div key={x} className="glass rounded-xl p-4">
                <p className="mb-2 font-mono text-xs text-primary">0{idx + 1}</p>
                <p>{x}</p>
              </div>
            ))}
          </div>
      ...
      <section>
        <Container>
          <SectionTitle eyebrow="Комплаенс" title="Безопасность и правовые основания" />
          <div className="glass rounded-2xl p-6 text-muted">
            <ul className="list-disc space-y-2 pl-5">
              <li>Работа с агрегированными и обезличенными данными в сценариях аналитики.</li>
              <li>Договорные контуры и разграничение ролей доступа.</li>
              <li>Соответствие требованиям 152-ФЗ и корпоративной ИБ-политике.</li>
            </ul>
          </div>
        </Container>
      </section>
